...
namespace Ioad {
  ...
  template <typename T>
  void AdiosWrapper::GetSync(adios2::Variable<T> var, T* data)
  {
  ...
  template <typename T>
  void AdiosWrapper::GetSync(std::string var_name, T* data)
  {
 ...
 template <typename T>
  ...
  template <typename T>
  void AdiosWrapper::DefineAttribute(const std::string &name, const T &value)
  {
     adios2::Attribute<T> attr =
          this->IO::InquireAttribute<T>(name);
      if (!attr) {
